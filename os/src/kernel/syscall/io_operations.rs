...
use crate::user_api::errno::EFAULT;
use crate::user_api::errno::EINVAL;
use crate::user_api::iovec::IoVec;
use crate::utilities::user_buffer::{validate_user_ptr, validate_user_ptr_mut};
...
pub fn sendfile(out_fd: usize, in_fd: usize, offset: *mut i64, count: usize) -> isize {
    ...
    let in_file = match task.lock().file_descriptor_table.get(in_fd) {
        Ok(f) => f,
        Err(e) => return e.to_errno(),
    };

    let out_file = match task.lock().file_descriptor_table.get(out_fd) {
        Ok(f) => f,
        Err(e) => return e.to_errno(),
    };
...
fn poll_with_timeout(
    fds: usize,
    nfds: usize,
    timeout: Option<crate::user_api::time::TimeSpec>,
) -> isize {
    use crate::arch::timer::{clock_freq, get_time};
    use crate::arch::trap::SumGuard;
    use crate::user_api::errno::{EINTR, EINVAL};
    ...
    loop {
...

        {
            ...
            for pollfd in pollfds.iter_mut() {
                ...
                let file = match task.lock().file_descriptor_table.get(pollfd.fd as usize) {
                    Ok(f) => f,
                    Err(_) => {
...
pub fn ppoll(fds: usize, nfds: usize, timeout: usize, _sigmask: usize) -> isize {
    ...
    let timeout_spec = if timeout == 0 {
        ...
        let _guard = SumGuard::new();
        unsafe {
            let timespec = timeout as *const crate::user_api::time::TimeSpec;
            let ts = *timespec;
...
pub fn pselect6(
    ...
    use crate::arch::trap::SumGuard;
    use crate::user_api::errno::EINVAL;
    use crate::user_api::time::TimeSpec;
...
pub fn select(
    ...
    use crate::arch::trap::SumGuard;
    use crate::user_api::errno::EINVAL;
    use crate::user_api::time::timeval;
...
fn select_common(
    ...
    use crate::arch::trap::SumGuard;
    use crate::kernel::current_task;
    use crate::user_api::errno::{EBADF, EINTR, EINVAL};
    use crate::user_api::select::FdSet;

    if nfds > crate::user_api::select::FD_SETSIZE {
        return -(EINVAL as isize);
    }
    ...
    let check_fds = || -> (isize, Option<FdSet>, Option<FdSet>, Option<FdSet>) {
        ...
        for fd in 0..nfds {
            ...
            let file = match task_lock.file_descriptor_table.get(fd) {
                Ok(f) => f,
                Err(_) => {
                    crate::pr_warn!(
...
pub fn poll(fds: usize, nfds: usize, timeout: i32) -> isize {
    let timeout_spec = if timeout < 0 {
        ...
        Some(crate::user_api::time::TimeSpec {
