...
use crate::memory::address::{PageNum, UsizeConvert, Vaddr, Vpn, VpnRange};
use crate::memory::memory_space::MmapFile;
use crate::memory::memory_space::mapping_area::AreaType;
use crate::memory::page_table::UniversalPTEFlag;
use crate::user_api::errno::{EACCES, EBADF, EEXIST, EINVAL, EIO, ENOMEM};
use crate::user_api::memory::{MAP_FAILED, MapFlags, ProtFlags};
...
pub fn mmap(addr: *mut c_void, len: usize, prot: i32, flags: i32, fd: i32, offset: i64) -> isize {
    ...
    let mmap_file = if !map_flags.contains(MapFlags::ANONYMOUS) {
        ...
        let file = match task.lock().file_descriptor_table.get(fd as usize) {
            Ok(f) => f,
            Err(_) => {
