...
use crate::{
    ...
    user_api::{
        ...
        filesystem::{AtFlags, F_OK, FileSystemType, LinuxStatFs, R_OK, W_OK, X_OK},
        time::TimeSpec,
    },
    virtual_fs::{
...
pub fn close(fd: usize) -> isize {
    ...
    if let Ok(file) = task_lock.file_descriptor_table.get(fd) {
        if file
            .as_any()
            .downcast_ref::<crate::network::socket::SocketFile>()
            .is_some()
        {
            crate::network::socket::unregister_socket_fd(tid, fd);
        }
    }

    match task_lock.file_descriptor_table.close(fd) {
        Ok(()) => 0,
        Err(e) => e.to_errno(),
    }
}
...
pub fn lseek(fd: usize, offset: isize, whence: usize) -> isize {
    // 获取文件对象
    let task = current_task();
    let file = match task.lock().file_descriptor_table.get(fd) {
        Ok(f) => f,
...
pub fn openat(dirfd: i32, pathname: *const c_char, flags: u32, mode: u32) -> isize {
    ...
    // 分配文件描述符
    let task = current_task();
    match task.lock().file_descriptor_table.alloc(file) {
        Ok(fd) => fd as isize,
        Err(e) => e.to_errno(),
    }
}
...
pub fn fstat(fd: usize, statbuf: *mut Stat) -> isize {
    ...
    // 转换为 Stat 结构
    let stat = crate::virtual_fs::Stat::from_metadata(&metadata);
...
pub fn getdents64(fd: usize, dirp: *mut u8, count: usize) -> isize {
    use crate::virtual_fs::{LinuxDirent64, inode_type_to_d_type};
...
pub fn statfs(path: *const c_char, buf: *mut LinuxStatFs) -> isize {
    ...
    use crate::virtual_fs::MOUNT_TABLE;
...
pub fn statx(
    ...
    if path_str.is_empty() && at_flags.contains(AtFlags::EMPTY_PATH) {
        ...
        let task = current_task();
        let file = match task.lock().file_descriptor_table.get(dirfd as usize) {
            Ok(f) => f,
            Err(e) => return e.to_errno(),
        };
        let metadata = match file.metadata() {
        ...
        let stx = crate::virtual_fs::Statx::from_metadata(&metadata);
        let _guard = SumGuard::new();
        unsafe {
...
pub fn mount(
    ...
    use crate::filesystem::ext4::Ext4FileSystem;
    use crate::filesystem::sysfs::find_block_device;
    use crate::filesystem::{init_dev, init_procfs, init_sysfs, mount_tmpfs};
    use crate::virtual_fs::{MOUNT_TABLE, MountFlags as VfsMountFlags};
    ...
    fn ensure_dir_exists(path: &str) -> Result<(), FsError> {
        use crate::virtual_fs::{FileMode, split_path, vfs_lookup};
...
/// 此调用只需刷新硬件写缓存。
pub fn sync() -> isize {
    use crate::kernel::syscall::utilities::flush_all_block_devices;
...
pub fn syncfs(fd: usize) -> isize {
    use crate::kernel::syscall::utilities::flush_block_device_by_fd;
...
pub fn fchownat(dirfd: i32, pathname: *const c_char, owner: u32, group: u32, flags: u32) -> isize {
    use crate::user_api::filesystem::AtFlags;
