...
use crate::user_api::errno::{EBADF, EINVAL, ENOTTY, EOPNOTSUPP};
use crate::user_api::io_control::*;
...
pub fn ioctl(fd: i32, request: u32, arg: usize) -> isize {
    ...
    let file = {
        ...
        match task_lock.file_descriptor_table.get(fd as usize) {
            Ok(f) => f,
            Err(_) => {
...
fn handle_fionbio(file: &alloc::sync::Arc<dyn crate::virtual_fs::File>, arg: usize) -> isize {
    unsafe {
        ...
        if value != 0 {
            flags |= crate::user_api::file_control::OpenFlags::O_NONBLOCK;
        } else {
            flags &= !crate::user_api::file_control::OpenFlags::O_NONBLOCK;
        }
...
/// FIONREAD - 获取可读字节数
fn handle_fionread(file: &alloc::sync::Arc<dyn crate::virtual_fs::File>, arg: usize) -> isize {
    unsafe {
        let _guard = SumGuard::new();
...
fn handle_fioasync(_file: &alloc::sync::Arc<dyn crate::virtual_fs::File>, arg: usize) -> isize {
    unsafe {
        let _guard = SumGuard::new();
        let value_ptr = arg as *const i32;
        if value_ptr.is_null() {
...
fn handle_tiocgpgrp(
    ...
    unsafe {
        crate::utilities::user_buffer::write_to_user(arg as *mut i32, pgid);
    }
...
fn handle_tiocsctty(_file: &alloc::sync::Arc<dyn crate::virtual_fs::File>, _arg: usize) -> isize {
    // TODO: 实现完整的控制终端管理
...
fn fs_error_to_errno(err: FsError) -> isize {
    match err {
        ...
        FsError::NotFound => -crate::user_api::errno::ENOENT as isize,
        FsError::PermissionDenied => -crate::user_api::errno::EACCES as isize,
        FsError::AlreadyExists => -crate::user_api::errno::EEXIST as isize,
        FsError::IoError => -crate::user_api::errno::EIO as isize,
        _ => -crate::user_api::errno::EIO as isize,
    }
}
