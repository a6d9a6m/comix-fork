...
use crate::{
    ...
    interprocess::{SignalHandlerTable, SignalPending, signal_pending},
    kernel::{
        ...
        syscall::utilities::{get_args_safe, get_path_safe},
...
pub fn exit(code: c_int) -> c_int {
    ...
    crate::kernel::task_control::cleanup_current_process_resources_on_exit();
...
pub fn clone(
    ...
    let (
        ...
        (
            ...
            task.file_descriptor_table.clone(),
            task.fs.clone(),
    ...
    let file_descriptor_table = if requested_flags.contains(CloneFlags::FILES) {
        file_descriptor_table
    } else {
        Arc::new(file_descriptor_table.clone_table())
    };
...
pub fn execve(
    ...
    let (argv_strings, envp_strings, exec_path_str) = {
        ...
        let dentry = match crate::virtual_fs::vfs_lookup(&path_str) {
            Ok(d) => d,
        ...
        if meta.inode_type != crate::virtual_fs::InodeType::File {
            return -EISDIR;
        }
    ...
    let exe_path = match crate::virtual_fs::vfs_lookup(&exec_path_str) {
        Ok(d) => d.full_path(),
...
pub fn get_pgid(pid: c_int) -> c_int {
    use crate::user_api::errno::ESRCH;
...
pub fn set_pgid(pid: c_int, pgid: c_int) -> c_int {
    use crate::user_api::errno::{EINVAL, EPERM, ESRCH};
...
fn do_execve_switch(
    ...
    task.lock().file_descriptor_table.close_exec();
