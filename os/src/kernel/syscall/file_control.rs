...
use crate::user_api::file_control::{FcntlCmd, FdFlags, FileStatusFlags, Flock, LockType};
use crate::virtual_fs::{FsError, OpenFlags, file_lock_manager};
use alloc::sync::Arc;
...
pub fn fcntl(fd: usize, cmd_raw: i32, arg: usize) -> isize {
    ...
    match cmd {
        FcntlCmd::GetFd => {
            // F_GETFD: 获取文件描述符标志
            match task.lock().file_descriptor_table.get_fd_flags(fd) {
        ...
        FcntlCmd::SetFd => {
            ...
            match task.lock().file_descriptor_table.set_fd_flags(fd, flags) {
                Ok(()) => 0,
                Err(e) => e.to_errno(),
            }
        }
...
fn fcntl_dupfd(
    task: &Arc<crate::synchronization::SpinLock<crate::kernel::task_control::TaskStruct>>,
    old_fd: usize,
    min_fd: usize,
    cloexec: bool,
) -> isize {
