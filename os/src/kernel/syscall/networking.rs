...
use crate::{
    ...
    user_api::{
        file_control::{FdFlags, OpenFlags},
...
pub fn bind(sockfd: i32, addr: *const u8, addrlen: u32) -> isize {
    ...
    let file = match task_lock.file_descriptor_table.get(sockfd as usize) {
        Ok(f) => f,
    ...
    match handle {
        SocketHandle::Tcp(_) => {
            use crate::network::socket::set_socket_local_endpoint;
        ...
        SocketHandle::Udp(h) => {
            ...
            if crate::network::socket::udp_attach_fd_to_port(
                tid,
                sockfd as usize,
                &file,
                h,
...
pub fn listen(sockfd: i32, backlog: i32) -> isize {
    ...
    match handle {
        SocketHandle::Tcp(h) => {
            use crate::network::socket::SocketFile;
            ...
            use crate::network::socket::get_socket_local_endpoint;
...
pub fn accept(sockfd: i32, addr: *mut u8, addrlen: *mut u32) -> isize {
    ...
    let file = match task.lock().file_descriptor_table.get(sockfd as usize) {
        Ok(f) => f,
        Err(_) => return -9, // EBADF
    };
    ...
    let is_nonblock = socket_file
        .flags()
        .contains(crate::user_api::file_control::OpenFlags::O_NONBLOCK);
    let backlog = socket_file.listen_backlog().max(1).min(128);

    loop {
        // 推进 loopback + 网络状态机
        crate::network::socket::poll_until_empty();
        ...
        if state != tcp::State::Listen && socket_file.listen_sockets_len() < backlog {
            ...
            use crate::network::socket::{update_socket_file_handle, update_socket_handle};
        ...
        if crate::interprocess::signal_interrupts_syscall(&task) {
            return -(crate::user_api::errno::EINTR as isize);
        }
    }
}
...
fn accept_return_conn(
    ...
    match task.lock().file_descriptor_table.alloc(conn) {
        Ok(fd) => {
...
pub fn connect(sockfd: i32, addr: *const u8, addrlen: u32) -> isize {
    ...
    use crate::network::socket::set_socket_remote_endpoint;
    ...
    match handle {
        SocketHandle::Tcp(h) => {
            ...
            if let Some(sf) = file
                .as_any()
                .downcast_ref::<crate::network::socket::SocketFile>()
            {
                sf.set_local_endpoint(local_endpoint);
            }
            drop(sockets);

            use crate::network::socket::tcp_connect;
            ...
            if !is_nonblock {
                use crate::network::socket::SOCKET_SET;
    ...
    loop {
        ...
        let (_tid, file) = {
            ...
            if sockfd < 0 {
                ...
                return -(crate::user_api::errno::EBADF as isize);
            }
            let file = match task_lock.file_descriptor_table.get(sockfd as usize) {
                Ok(f) => f,
                Err(_) => {
...
pub fn close_sock(sockfd: i32) -> isize {
    ...
    match task_lock.file_descriptor_table.close(sockfd as usize) {
        Ok(_) => 0,
        Err(_) => -9,
    }
}
...
fn get_interface_stats(ifname: *const c_char, stats: *mut u8, size: usize) -> isize {
    ...
    use crate::user_api::errno::{EFAULT, EINVAL, ENODEV};
...
pub fn getifaddrs(ifap: *mut *mut u8) -> isize {
    use crate::arch::trap::SumGuard;
    use crate::user_api::errno::{EFAULT, ENOMEM};
    ...
    let (user_mem_start, map_len) = {
        ...
        use crate::kernel::syscall::memory::mmap;
...
pub fn freeifaddrs(ifa: *mut u8) -> isize {
    use crate::arch::trap::SumGuard;
    use crate::kernel::syscall::memory::munmap;
    use crate::user_api::errno::EINVAL;
...
pub fn setsockopt(sockfd: i32, level: i32, optname: i32, optval: *const u8, optlen: u32) -> isize {
    use crate::arch::trap::SumGuard;
    use crate::user_api::errno::{EBADF, EINVAL, ENOPROTOOPT, ENOTSOCK};
    use crate::user_api::socket::*;
...
pub fn accept4(sockfd: i32, addr: *mut u8, addrlen: *mut u32, flags: i32) -> isize {
    ...
    if flags & SOCK_CLOEXEC != 0 {
        let task_lock = task.lock();
        if let Ok(old_flags) = task_lock.file_descriptor_table.get_fd_flags(fd_usize) {
            let _ = task_lock
    ...
    if flags & SOCK_NONBLOCK != 0 {
        let task_lock = task.lock();
        if let Ok(file) = task_lock.file_descriptor_table.get(fd_usize) {
...
pub fn sendto(
    ...
    use crate::network::socket::socket_sendto;
    let result = {
        let _guard = SumGuard::new();
        let data = unsafe { core::slice::from_raw_parts(buf, len) };
...
pub fn shutdown(sockfd: i32, how: i32) -> isize {
    ...
    use crate::network::socket::{socket_shutdown_read, socket_shutdown_write};
...
pub fn getpeername(sockfd: i32, addr: *mut u8, addrlen: *mut u32) -> isize {
    ...
    let remote_endpoint = match handle {
        ...
        SocketHandle::Udp(_) => {
            ...
            use crate::network::socket::get_socket_remote_endpoint;
