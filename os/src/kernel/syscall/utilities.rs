...
use crate::{
    kernel::current_task,
    user_api::{errno::EINVAL, logging::SyslogAction},
    virtual_fs::{
...
pub fn resolve_at_path(dirfd: i32, path: &str) -> Result<Option<Arc<Dentry>>, FsError> {
    let base_dentry = if path.starts_with('/') {
        get_root_dentry()?
    } else if dirfd == super::filesystem::AT_FDCWD {
        current_task()
            .lock()
            .fs
            .lock()
        ...
        let file = task.lock().file_descriptor_table.get(dirfd as usize)?;
...
pub fn get_first_block_device() -> Result<Arc<dyn crate::device::block::BlockDriver>, i32> {
    ...
    use crate::user_api::errno::ENODEV;
...
pub fn flush_all_block_devices() -> Result<(), isize> {
    use crate::{device::BLK_DRIVERS, user_api::errno::EIO};
...
pub fn flush_block_device_by_fd(fd: usize) -> Result<(), isize> {
    use crate::{user_api::errno::EIO, virtual_fs::MOUNT_TABLE};
...
pub fn resolve_at_path_with_flags(
    ...
    use crate::virtual_fs::{vfs_lookup_no_follow, vfs_lookup_no_follow_from};
    ...
    if follow_symlink {
        ...
        if path.starts_with('/') {
            ...
            // 相对路径，需要从 dirfd 开始
            let base_dentry = if dirfd == super::filesystem::AT_FDCWD {
                // 使用当前工作目录
                current_task()
                    .lock()
                    .fs
