...
use crate::{
    ...
    kernel::{
        current_task,
        syscall::utilities::{check_syslog_permission, validate_syslog_args},
...
pub fn syslog(type_: i32, bufp: *mut u8, len: i32) -> isize {
    ...
    match action {
        ...
        SyslogAction::ReadAll => {
            // ReadAll: 非破坏性读取（保留条目）
            use crate::logging::{log_reader_index, log_writer_index, peek_log};
        ...
        SyslogAction::SizeUnread => {
            // 返回未读日志的精确字节数
            use crate::logging::log_unread_bytes;
            log_unread_bytes() as isize
        }

        SyslogAction::SizeBuffer => {
            // 返回日志缓冲区的总大小
            use crate::logging::GLOBAL_LOG_BUFFER_SIZE;
