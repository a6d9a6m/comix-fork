...
use crate::user_api::signal::NUM_SIGCHLD;
use crate::{
    arch::trap::restore,
    kernel::{cpu::current_cpu, schedule},
    virtual_fs::{FDTable, File, FsError},
};
...
pub fn cleanup_current_process_resources_on_exit() {
    ...
    let file_descriptor_table = { task.lock().file_descriptor_table.clone() };
    let open = file_descriptor_table.take_all();
    for (fd, file) in open {
        if file
            .as_any()
...
pub fn current_fd_table() -> Arc<FDTable> {
    current_task().lock().file_descriptor_table.clone()
}
