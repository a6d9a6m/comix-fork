...
use crate::{
    arch::{
        kernel::{context::Context, task_context::setup_stack_layout},
        trap::TrapFrame,
    },
    interprocess::{SignalHandlerTable, SignalPending},
    kernel::{
        WaitQueue,
        task_control::{forkret, task_state::TaskState},
    },
    memory::{
...
pub struct Task {
    ...
    // === 文件系统 ===
    /// 文件描述符表
    pub file_descriptor_table: Arc<FDTable>,
    /// 文件系统信息
...
impl Task {
    ...
    pub fn ktask_create(
        ...
        file_descriptor_table: Arc<FDTable>,
    ...
    pub fn execve(
        ...
        let new_fd_table = self.file_descriptor_table.clone_table();
        new_fd_table.close_exec();
        self.file_descriptor_table = Arc::new(new_fd_table);
    ...
    pub fn new_dummy_task(tid: u32) -> Self {
        use crate::{
            ...
            user_api::resource::INIT_RLIMITS,
        };
        let kstack_tracker =
