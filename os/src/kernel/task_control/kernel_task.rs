...
use crate::{
    arch::{interrupts::disable_interrupts, trap::restore},
    kernel::{
        TaskState,
        cpu::current_cpu,
        scheduler::Scheduler,
        task_control::{TASK_MANAGER, TaskStruct, task_registry::TaskManagerTrait},
    },
    memory::frame_allocator::{alloc_contig_frames, alloc_frame},
...
pub fn kthread_spawn(entry_point: fn()) -> u32 {
    ...
    let (pid, ppid, signal_handlers, blocked, signal, uts, rlimit, file_descriptor_table, fs) = {
        ...
        (
            ...
            cur_task.file_descriptor_table.clone(),
            cur_task.fs.clone(),
        )
    };
...
pub fn kernel_execve(path: &str, argv: &[&str], envp: &[&str]) -> ! {
    ...
    #[cfg(target_arch = "loongarch64")]
    {
        use crate::memory::address::PageNum;
        use crate::memory::address::{UsizeConvert, Vaddr};
        let tlbrent: usize;
        let crmd: usize;
        let pgdl: usize;
        let pgdh: usize;
        ...
        let tlbr_entry_paddr =
            unsafe { crate::arch::memory::vaddr_to_paddr(tlbr_entry_vaddr) } & !0xfff;
        let tlbr_entry_dm_vaddr = crate::arch::memory::paddr_to_vaddr(tlbr_entry_paddr);
        crate::pr_debug!(
...
mod tests {
    ...
    use crate::{
        kassert,
        kernel::task_control::{SharedTask, TASK_MANAGER},
        test_case,
    };
    ...
    //     let tid = TASK_MANAGER.lock().allocate_tid();
    //     let kstack_tracker =
    //         crate::memory::frame_allocator::physical_page_alloc_contiguous(1).expect("alloc kstack");
    //     let trap_frame_tracker =
    //         crate::memory::frame_allocator::physical_page_alloc().expect("alloc trap_frame");
    //     let task = TaskStruct::ktask_create(
    //         tid,
    //         tid,
    //         0,
