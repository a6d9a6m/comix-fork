...
use crate::virtual_fs::{File, FsError, InodeMetadata};
...
pub struct NetIfaceWrapper {
    device: SpinLock<crate::network::interface::NetDeviceAdapter>,
...
use crate::user_api::file_control::OpenFlags;
use crate::user_api::socket::SocketOptions;
...
struct UdpPortEntry {
    ...
    sockets: alloc::vec::Vec<alloc::sync::Weak<dyn crate::virtual_fs::File>>,
}
...
pub fn set_socket_local_endpoint(
    file: &Arc<dyn crate::virtual_fs::File>,
    endpoint: IpEndpoint,
) -> Result<(), ()> {
    let any = file.as_any();
...
pub fn get_socket_local_endpoint(file: &Arc<dyn crate::virtual_fs::File>) -> Option<IpEndpoint> {
    let any = file.as_any();
...
pub fn get_socket_remote_endpoint(file: &Arc<dyn crate::virtual_fs::File>) -> Option<IpEndpoint> {
    let any = file.as_any();
    any.downcast_ref::<SocketFile>()
...
pub fn socket_shutdown_read(file: &Arc<dyn crate::virtual_fs::File>) {
...
pub fn socket_shutdown_write(file: &Arc<dyn crate::virtual_fs::File>) {
    if let Some(socket_file) = file.as_any().downcast_ref::<SocketFile>() {
...
pub fn init_network(mut smoltcp_iface: crate::network::interface::SmoltcpInterface) {
...
fn udp_dispatch_drain_locked(sockets: &mut SocketSet<'static>) -> bool {
    ...
    for (port, entry) in ports.iter_mut() {
        ...
        while socket.can_recv() {
            ...
            let mut target: Option<alloc::sync::Arc<dyn crate::virtual_fs::File>> = None;
            let mut fallback: Option<alloc::sync::Arc<dyn crate::virtual_fs::File>> = None;
