use crate::{
    filesystem::proc::ProcInode,
    virtual_fs::{FileMode, FileSystem, FsError, Inode, StatFs},
};
use alloc::sync::Arc;
...
impl ProcFS {
    ...
    pub fn init_tree(self: &Arc<Self>) -> Result<(), FsError> {
        use crate::filesystem::proc::generators::{
        ...
        let psmem = ProcInode::new_dynamic_file(
            "psmem",
            alloc::sync::Arc::new(crate::filesystem::proc::generators::PsmemGenerator),
