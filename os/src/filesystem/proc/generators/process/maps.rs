...
use crate::{
    config::PAGE_SIZE,
    filesystem::proc::ContentGenerator,
    kernel::TaskStruct,
    memory::address::{PageNum, UsizeConvert},
    memory::memory_space::mapping_area::{AreaType, MapType},
...
impl ContentGenerator for MapsGenerator {
    fn generate(&self) -> Result<Vec<u8>, FsError> {
        ...
        for a in areas {
            ...
            let r = if perm.contains(crate::memory::page_table::UniversalPTEFlag::READABLE) {
                'r'
            } else {
                '-'
            };
            let w = if perm.contains(crate::memory::page_table::UniversalPTEFlag::WRITEABLE) {
                'w'
            } else {
                '-'
            };
            let x = if perm.contains(crate::memory::page_table::UniversalPTEFlag::EXECUTABLE) {
                'x'
            } else {
                '-'
            };
