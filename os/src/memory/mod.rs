...
use crate::arch::memory::vaddr_to_paddr;
use crate::config::{MEMORY_END, PAGE_SIZE};
use crate::earlyprintln;
use crate::memory::address::{Ppn, UsizeConvert};
...
pub fn init() -> alloc::sync::Arc<crate::synchronization::SpinLock<memory_space::MemorySpace>> {
    // 1. 初始化物理帧分配器
...
pub fn activate(root_ppn: Ppn) {
    use crate::memory::page_table::PageTableInner as PageTableInnerTrait;
    // 调用特定架构的页表激活函数，例如在 RISC-V 上设置 SATP 寄存器。
    crate::arch::memory::PageTableInner::activate(root_ppn);
}
