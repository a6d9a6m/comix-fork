...
#![allow(dead_code)]
use crate::arch::interrupts::{are_interrupts_enabled, disable_interrupts, enable_interrupts, read_and_enable_interrupts, restore_interrupts};
...
macro_rules! test_case {
    ...
    (
        ...
        fn $func_name() {
            ...
            let failed_before = $crate::tests::TEST_FAILED.load(core::sync::atomic::Ordering::SeqCst);

            // 创建guard，它会在作用域结束时自动清理
            let _guard = $crate::tests::TestEnvironment::$env.guard();

            $body // 执行测试

            let failed_after = $crate::tests::TEST_FAILED.load(core::sync::atomic::Ordering::SeqCst);
            let failed_count = failed_after - failed_before;

            unsafe {
                for i in failed_before..$crate::tests::FAILED_INDEX {
                    if let Some(fail) = &$crate::tests::FAILED_LIST[i] {
                        println!(
