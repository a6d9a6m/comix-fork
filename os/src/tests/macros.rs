...
macro_rules! kassert {
    ($cond:expr) => {{
        if !$cond {
            ...
            let fa =
                $crate::tests::macros::FailedAssertion::new(stringify!($cond), file!(), line!());
            // 调用安全封装函数（内部负责 unsafe）
            $crate::tests::macros::record_failed_assertion(fa);
        }
    }};
}
...
macro_rules! test_case {
    (
        ...
        fn $func_name() {
            ...
            let _guard = $crate::tests::guard::TestEnvGuard::enter($env);

            println!("\x1b[33m[Running test: {} (with env)]\x1b[0m", stringify!($name));
            let failed_before = $crate::tests::macros::TEST_FAILED.load(core::sync::atomic::Ordering::SeqCst);

            $body

            let failed_after = $crate::tests::macros::TEST_FAILED.load(core::sync::atomic::Ordering::SeqCst);
    ...
    (
        ...
        fn $func_name() {
            ...
            unsafe {
                for i in failed_before..$crate::tests::macros::FAILED_INDEX {
                    if let Some(fail) = $crate::tests::macros::FAILED_LIST[i] {
                        $crate::println!(
