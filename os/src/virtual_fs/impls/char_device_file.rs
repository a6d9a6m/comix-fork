...
use crate::user_api::io_control::Termios;
use crate::virtual_fs::dev::{major, minor};
use crate::virtual_fs::device_number::{chrdev_major, get_chrdev_driver, misc_minor};
use crate::virtual_fs::{Dentry, File, FsError, Inode, InodeMetadata, OpenFlags, SeekWhence};
use alloc::sync::Arc;
...
pub struct CharDeviceFile {
    ...
    /// 终端窗口大小（用于 TTY 设备）
    winsize: SpinLock<crate::user_api::io_control::WinSize>,
}
...
impl CharDeviceFile {
    ...
    pub fn new(dentry: Arc<Dentry>, flags: OpenFlags) -> Result<Self, FsError> {
        ...
        Ok(Self {
            ...
            winsize: SpinLock::new(crate::user_api::io_control::WinSize {
                ws_row: 24,
                ws_col: 80,
                ws_xpixel: 0,
                ws_ypixel: 0,
...
impl CharDeviceFile {
    ...
    fn console_ioctl(&self, request: u32, arg: usize) -> Result<isize, FsError> {
        ...
        use crate::user_api::errno::{EINVAL, ENOTTY};
        ...
        match request {
            ...
            TIOCGWINSZ => {
                ...
                unsafe {
                    let _guard = SumGuard::new();
                    let winsize_ptr = arg as *mut crate::user_api::io_control::WinSize;
                    ...
                    core::ptr::write_bytes(
                        ...
                        0,
                        core::mem::size_of::<crate::user_api::io_control::WinSize>(),
                    );
            ...
            TIOCSWINSZ => {
                ...
                {
                    let _guard = SumGuard::new();
                    let winsize_ptr = arg as *const crate::user_api::io_control::WinSize;
                    if winsize_ptr.is_null() {
                        return Ok(-EINVAL as isize);
                    }
    ...
    fn misc_ioctl(&self, request: u32, arg: usize) -> Result<isize, FsError> {
        ...
        use crate::user_api::io_control::*;
        use crate::virtual_fs::dev::minor;
