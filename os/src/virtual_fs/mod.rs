...
//! - [`mount`] - 挂载表管理和挂载点栈
//! - [`file_descriptor_table`] - 进程级文件描述符表
//! - [`file_locking`] - POSIX 文件锁管理器
//! - [`filesystem`] - 文件系统抽象接口
//! - [`impls`] - 具体文件类型实现（RegFile、PipeFile 等）
//! - [`error`] - VFS 错误类型定义
//! - [`dev`]/[`device_number`] - 设备号管理和驱动注册
//!
//! # 设计概览
//!
//! ## 分层架构
...
//!
//! ```rust
//! use virtual_fs::{vfs_lookup, RegFile, OpenFlags};
//! use alloc::sync::Arc;
//!
//! // 1. 查找文件
...
//!
//! ```rust
//! use virtual_fs::{MOUNT_TABLE, MountFlags};
//!
//! // 创建文件系统
...
//!
//! ```rust
//! use virtual_fs::PipeFile;
//!
//! let (read_file, write_file) = PipeFile::create_pipe()?;
//!
//! // 分配文件描述符
//! let read_fd = file_descriptor_table.alloc(read_file)?;
//! let write_fd = file_descriptor_table.alloc(write_file)?;
//!
//! // 父子进程通过管道通信
//! ```
...
pub use crate::user_api::file_control::{FdFlags, OpenFlags, SeekWhence};
pub use crate::user_api::filesystem::{LinuxDirent64, Stat, Statx};
pub use crate::user_api::time::TimeSpec;
