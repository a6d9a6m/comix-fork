...
test_case!(test_fdtable_with_mixed_files, {
    ...
    let disk_file = create_test_file("test.txt", inode, OpenFlags::O_RDONLY);
    let fd1 = file_descriptor_table.alloc(disk_file).unwrap();

    // 添加 PipeFile
    let (pipe_read, pipe_write) = PipeFile::create_pair();
    let fd2 = file_descriptor_table
        .alloc(Arc::new(pipe_read) as Arc<dyn File>)
        .unwrap();
    let fd3 = file_descriptor_table
        .alloc(Arc::new(pipe_write) as Arc<dyn File>)
        .unwrap();

    // 验证可以取出
    kassert!(file_descriptor_table.get(fd1).is_ok());
    kassert!(file_descriptor_table.get(fd2).is_ok());
    kassert!(file_descriptor_table.get(fd3).is_ok());
...
test_case!(test_read_write_through_fdtable, {
    ...
    // 创建管道
    let (pipe_read, pipe_write) = PipeFile::create_pair();
    let fd_read = file_descriptor_table
        .alloc(Arc::new(pipe_read) as Arc<dyn File>)
        .unwrap();
    let fd_write = file_descriptor_table
        .alloc(Arc::new(pipe_write) as Arc<dyn File>)
        .unwrap();

    // 通过 fd 写入
    let write_file = file_descriptor_table.get(fd_write).unwrap();
    write_file.write(b"via fdtable").unwrap();

    // 通过 fd 读取
    let read_file = file_descriptor_table.get(fd_read).unwrap();
    let mut buf = [0u8; 11];
