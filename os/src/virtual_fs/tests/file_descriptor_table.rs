...
test_case!(test_fdtable_create, {
    // 创建 FDTable
    let file_descriptor_table = FDTable::new();

    // FDTable 应该初始化为空
    let result = file_descriptor_table.get(0);
...
test_case!(test_fdtable_get, {
    ...
    // 分配并获取 FD
    let fd = file_descriptor_table.alloc(file.clone()).unwrap();
    let retrieved = file_descriptor_table.get(fd);
...
test_case!(test_fdtable_close, {
    ...
    // 关闭 FD
    let result = file_descriptor_table.close(fd);
...
test_case!(test_fdtable_dup, {
    ...
    // 复制 FD
    let new_fd = file_descriptor_table.dup(fd).unwrap();
    kassert!(new_fd != fd);

    // 两个 FD 应该指向同一个文件对象
    let file1 = file_descriptor_table.get(fd).unwrap();
    let file2 = file_descriptor_table.get(new_fd).unwrap();
...
test_case!(test_fdtable_dup2, {
    ...
    // 分配两个 FD
    let fd1 = file_descriptor_table.alloc(file1.clone()).unwrap();
    let fd2 = file_descriptor_table.alloc(file2).unwrap();

    // dup2: 将 fd1 复制到 fd2
    let result = file_descriptor_table.dup2(fd1, fd2).unwrap();
    kassert!(result == fd2);

    // fd2 现在应该指向 file1
    let retrieved = file_descriptor_table.get(fd2).unwrap();
...
test_case!(test_fdtable_install_at, {
    ...
    // 在指定位置安装文件
    let result = file_descriptor_table.install_at(5, file.clone());
    kassert!(result.is_ok());

    // 验证
    let retrieved = file_descriptor_table.get(5);
    kassert!(retrieved.is_ok());
...
test_case!(test_fdtable_clone, {
    ...
    // 克隆 FDTable
    let cloned = file_descriptor_table.clone_table();
...
test_case!(test_fdtable_get_invalid_fd, {
    ...
    // 获取无效的 FD
    let retrieved = file_descriptor_table.get(99);
    kassert!(retrieved.is_err());
});
...
test_case!(test_fdtable_close_invalid_fd, {
    ...
    // 关闭无效的 FD
    let result = file_descriptor_table.close(99);
    kassert!(result.is_err());
...
test_case!(test_fdtable_dup_invalid_fd, {
    ...
    // 复制无效的 FD
    let result = file_descriptor_table.dup(99);
    kassert!(result.is_err());
    kassert!(matches!(result, Err(FsError::BadFileDescriptor)));
});
...
test_case!(test_fdtable_alloc_multiple, {
    ...
    for i in 0..5 {
        ...
        let result = file_descriptor_table.alloc(file);
        kassert!(result.is_ok());
    }
});
