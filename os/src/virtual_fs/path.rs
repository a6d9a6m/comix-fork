...
//!
//! ```rust
//! use virtual_fs::{parse_path, normalize_path, PathComponent};
//!
//! // 解析路径
...
//!
//! ```rust
//! use virtual_fs::{vfs_lookup, vfs_lookup_no_follow};
//!
//! // 创建符号链接 /link → /target
...
use crate::virtual_fs::{Dentry, FsError, InodeType, get_root_dentry};
    ...
    match component {
        ...
        PathComponent::Normal(name) => {
            ...
            if child_dentry.inode.cacheable() {
                ...
                // 4. 加入全局缓存
                crate::virtual_fs::DENTRY_CACHE.insert(&child_dentry);
            } else {
...
fn check_mount_point(dentry: Arc<Dentry>) -> Result<Arc<Dentry>, FsError> {
    ...
    if let Some(mount_point) = crate::virtual_fs::MOUNT_TABLE.find_mount(&full_path) {
