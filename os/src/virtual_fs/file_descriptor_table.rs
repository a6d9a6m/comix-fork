...
//!
//! ```rust
//! let fd1 = file_descriptor_table.alloc(file1)?;  // 返回 3
//! let fd2 = file_descriptor_table.alloc(file2)?;  // 返回 4
//! file_descriptor_table.close(3)?;
//! let fd3 = file_descriptor_table.alloc(file3)?;  // 返回 3 (重用)
//! ```
//!
//! ### dup 语义
//!
...
//!
//! ```rust
//! file_descriptor_table.close_exec();  // exec 前调用
//! ```
//!
//! # 并发安全
//!
...
//!
//! ```rust
//! use virtual_fs::{FDTable, RegFile};
//!
//! let file_descriptor_table = FDTable::new();
//!
//! // 分配文件描述符
//! let file = Arc::new(RegFile::new(dentry, OpenFlags::O_RDONLY));
//! let fd = file_descriptor_table.alloc(file)?;
//!
//! // 访问文件
//! let file = file_descriptor_table.get(fd)?;
//! file.read(&mut buf)?;
//!
//! // 关闭文件描述符
//! file_descriptor_table.close(fd)?;
//! ```
//!
//! ## dup 操作
//!
//! ```rust
//! // dup: 复制到最小可用 FD
//! let new_fd = file_descriptor_table.dup(old_fd)?;
//!
//! // dup2: 复制到指定 FD
//! file_descriptor_table.dup2(old_fd, new_fd)?;
//!
//! // dup3: dup2 + 设置标志
//! file_descriptor_table.dup3(old_fd, new_fd, OpenFlags::O_CLOEXEC)?;
//! ```
//!
//! ## 重定向示例
//!
...
//! let fd = file_descriptor_table.alloc(file)?;
//! file_descriptor_table.dup2(fd, 1)?;  // 1 = stdout
//! file_descriptor_table.close(fd)?;
//! // 现在所有 println! 都会写到文件
//! ```
...
use crate::user_api::file_control::{FdFlags, OpenFlags};
use crate::virtual_fs::{File, FsError};
use alloc::sync::Arc;
use alloc::vec::Vec;
use core::fmt;
